...
| **B = 7** (マイナー) | **【新機能】** 任意スキーマ層を追加（詳細は下記バージョン履歴 `4.7.0` を参照） |
| **C = 1** (ビルド) | **【内部リファクタリング・挙動無変更】** 今後実装する「全データオンメモリ ON/OFF 切り替え」機能の下地として、`db_engine` 内部の記憶域を整理。列ごとの値を複製して保持していた `ColumnStore` の二重管理を廃止し `Database.records` を唯一の実体にした（メモリ使用量の削減にもなる）。`Database::get`/`get_by_label`/`list_all`/`search_by_column`/`get_by_index` の戻り値を参照（`&Record`）から所有値（`Record`）に変更（`sql_engine`/`db_client`/`db_ffi` 側もこれに追随）。`.kdb` の `KdbFile::append_record`/`compact` が書き込み先バイトオフセットを返すようにし、`read_record_at` で1レコードだけを再読込できるようにした（今後の容量制限機能で使うランダムアクセス読み込みの基盤。`.kdb` ファイル形式自体は変更なし）。UPDATE時に列の値が変わった/削除された場合の二次インデックスの更新漏れも合わせて修正。ユーザーから見えるSQL構文・REST API・Web UIの挙動に変更は無く、既存の全テスト（`cargo test --workspace`）が無変更で成功することを確認済み |
