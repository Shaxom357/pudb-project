...
| **C = 1** (ビルド) | **【バグ修正・UIのみ】** Web 管理 UI（`/ui`）の設定・ログ・レコード一覧・DB Info の各ビューが縦スクロールできず、画面に収まらない部分（設定画面では「スキーマ管理」「全データオンメモリ」カード）が見えなくなっていた不具合を修正。`showView()` が表示中ビューの `display` を `''`（＝`block`）にしていたため、ビューが持つ入れ子フレックスのスクロール領域（`.info-panel` 等の `flex:1; overflow:auto`）が高さ制約を失い、外側の `overflow:hidden` でクリップされていた。`display:'flex'` を設定するよう変更し、`#viewRecords` にも他ビューと同じ `flex:1; overflow:hidden; flex-direction:column` を付与して統一。`ui.html` のみの変更で、API・SQL・ストレージ形式・`kdb` CLI の挙動には影響しない。全クレートの `Cargo.toml` を `4.9.1` に同期 |
