...
| **B = 8** (マイナー) | **【新機能】** 「全データオンメモリ」の ON/OFF を切り替え可能にした（既定は ON ＝ 従来どおり全レコードをメモリ常駐・オーバーヘッドなし）。OFF にすると容量制限モードになり、上限（絶対バイト数 `500MB`/`5GB` または搭載メモリに対する割合 `20%`）を超える分は、直近使われていない行の列データから `.kdb` へ退避してメモリ使用量を抑える（アクセス時に `.kdb` から単体で読み直し、再アクセスで立ち退き順が更新される＝実質LRU）。`db_engine` に `MemoryPolicy`/`MemorySizeSpec`/`Database::set_memory_policy`/`memory_stats` を新設し、公開メソッドのシグネチャは変えずに内部可変キャッシュ（`Mutex<MemCache>`）で実現。合わせて `db_engine` 内部の列データ二重管理を廃止（`ColumnStore` はラベル索引のみ担当、`records` が単一の実体）、参照系メソッドの戻り値を所有値化、`.kdb` に `read_record_at`（バイトオフセット指定の単体読み直し）を追加（`.kdb` フォーマット自体は無変更）。`GET`/`PUT /settings` に `all_in_memory`・`memory_limit`・`memory_limit_spec` を追加（変更は**管理者ロール `kagura` のみ**・**`.kdb` モードのみ**、設定はサイドカーファイル `<DB_FILE>.memory.json` に保存し再起動をまたいで保持）。`kdb in-memory-mode enable|disable` / `kdb in-memory-size <値>` を新設。Web UI の設定ビューにトグルと上限入力を追加。`db_engine`/`db_client` にユニット・統合テストを追加（新規17件）。全クレートの `Cargo.toml` を `4.8.0` に同期 |
