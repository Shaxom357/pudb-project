...
| **B = 2** (マイナー) | **【新機能】** SQL のラベル指定における `'label.<name>'` 文字列リテラル形式を、従来 `INSERT` 専用だった箇所から `SELECT`（`FROM`句、`AND`/`OR`混在可）・`UPDATE`（データ更新の対象ラベル、`UPDATE LABEL` の旧名・新名）・`DELETE`（データ削除の対象ラベル、`DELETE LABEL` のラベル名）へ拡張。識別子形式（英数字・`_`のみ、[parser.rs](sql_engine/src/parser.rs) の `read_ident`）では書けない `:` などを含むラベル（`country:Japan` のような namespace 付きタグ。REST API/FFI 経由では以前から保存・絞り込み可能だったが `/sql` からは `Unexpected character: ':'` で弾かれていた）が SQL から直接指定できるようになった。`'label.*'` は `label.*` と同じ意味（`INSERT`・`UPDATE LABEL`・`DELETE LABEL` では従来どおり不可）。字句解析（トークナイザ）自体は無変更（文字列リテラルはもともと任意の文字を受け付ける）で、`parse_label_target`／`parse_label_ref` にリテラル分岐を追加しただけ。`sql_engine` のパーサーのみの変更で、`db_engine`/`.kdb` バイナリフォーマット・既存の識別子形式クエリ・REST サーバー・Web UI・`kdb` CLI の挙動には影響しない。`sql_engine` のユニットテストを追加（計70件）。全クレートの `Cargo.toml` バージョンを `4.2.0` に同期 |
