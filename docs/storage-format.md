...
`BACKUP TO` は `.kdb` を**復号せず、暗号化バイナリのまま** `.kbak` アーカイブに収めます
（ヘッダーの `salt` も一緒に運ばれます）。マスターキー本体は含まれないため、`.kbak` から
復元するには作成時と同じマスターキーが必要です。復元先のマスターキーが異なる場合は、
旧キーで復号し復元先のキーで再暗号化（リキー。新しいランダム `salt` でヘッダーを作り直す）
します。詳細は [backup.md](backup.md) を参照してください。
