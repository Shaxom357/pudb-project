...
- ✅ **バックアップ／復元**: `.kdb` 本体・サイドカーファイル・`auth.json` を1つの `.kbak` アーカイブ（独自コンテナ・SHA-256 検証・`manifest.json` 付き）にまとめる。SQL `BACKUP TO '...' [WITH KEY] [WITHOUT AUTH]` / `RESTORE FROM '...' [OLD KEY '...']`、`kdb backup` / `kdb restore` / `kdb keyinfo` CLI（サーバー経由＋`--direct`）。**管理者ロール `kagura` のみ**・トランザクション中は `409`。マスターキーは非同梱が既定（`WITH KEY` で同梱可）。復元先のキーが作成時と異なればリキー（再暗号化）。`RESTORE` は現ファイルを退避してから置き換え、反映にはサーバー再起動が必要（適用後は書き込み系リクエストを `409`）。メジャーバージョン非互換の `.kbak` は拒否。保存先は世代管理（`<DB_FILE>.backup.json`）で剪定。マスターキー表示 `GET /settings/master-key`（管理者のみ・監査ログにキー値は残さない）。詳細は [backup.md](backup.md)
